...
BASE_ARCHIVE_DIR = Path('D:/pdd/客服绩效文件存档月度')  # 基础存档目录
MERGED_FILES_DIR = Path('D:/pdd/合并文件/客服绩效文件存档月度')  # 合并文件存储目录
SHEET = 0
...
TODAY_STR = last_month_start.strftime('%Y-%m-%d') + '_' + last_month_end.strftime('%Y-%m-%d')

def update_header_once(df: pd.DataFrame):
    """固定用 O 列（索引 16）作为状态列，每周第一天把表头改成 本周日期+下载状态"""
    new_status = f'{TODAY_STR}客服绩效下载状态'
    old_header = df.columns[16]          # O 列当前名字
...
def fetch_download_link(cookie: str):
    ...
    # 00:00:00 时间戳（秒）
    ts_start = int(datetime.combine(last_month_start, datetime.min.time()).timestamp())

    # 23:59:59 时间戳（秒）
    ts_end = int(datetime.combine(last_month_end, datetime.max.time()).timestamp())
...
def upload_merged_file_to_minio(merged_file_path: str, date_str: str = None) -> bool:
    ...
    try:
        ...
        # 构建MinIO路径：ods/pdd/pdd_kpi_monthly/dt=日期/merged_kpi_data.parquet
        minio_path = f"ods/pdd/pdd_kpi_monthly/dt={date_str}/merged_kpi_data.parquet"
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                print(f"[成功] 成功上传合并文件到MinIO月度: {minio_path}")
                return True
            else:
                print(f"[错误] MinIO上传失败: {result.get('message', '未知错误')}")
                return False
        else:
            print(f"[错误] MinIO API请求失败月度: {response.status_code} - {response.text}")
            return False
            
    except Exception as e:
        print(f"[错误] 上传合并文件到MinIO月度时出错: {str(e)}")
        return False
...
if __name__ == '__main__':
    ...
    try:
        refresh_dataset_response = requests.post(
            ...
            headers={"Content-Type": "application/json"},
            json={"dataset_path": "minio.warehouse.ods.pdd.pdd_kpi_monthly"}
        )
