...
# 配置
BASE_ARCHIVE_DIR = Path('D:/yingdao/erp/商品库存')  # 商品库存存档目录
MERGED_FILES_DIR = Path('D:/yingdao/erp/合并表格/商品库存')  # 合并文件存储目录
...
ERP_STOCK_URL = "https://www.erp321.com/app/item/SkuStock/WmsSkuStock.aspx"
...
class ERPStockCollector:
    ...
    def get_dynamic_params(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                # 直接导航到商品库存页面
                stock_url = f"{ERP_STOCK_URL}?_c=jst-epaas"
                print(f"导航到商品库存页面: {stock_url}")
                ...
                # 获取Cookie
                cookies = context.cookies(ERP_STOCK_URL)
    ...
    def fetch_stock_data(self, page_index: int = 1):
        """
        获取商品库存数据
        """
        ...
        # 构建URL - 不使用urlencode，直接拼接
        url = f"{ERP_STOCK_URL}?_c=jst-epaas&ts___={self.ts}&am___=LoadDataToJSON"

        # 构建请求头 - 将Cookie放在headers中
        headers = {
            'accept': '*/*',
        ...
        # 使用列表形式构建form_data，以支持重复的key（如qty）
        form_data = [
        ...
        try:
            print(f"发送商品库存数据请求 (页码: {page_index})...")
            print(f"请求URL: {url}")
            print(f"请求体: {form_data}")
    ...
    def collect_all_data(self):
        ...
        while True:
            # 获取当前页数据
            response_text = self.fetch_stock_data(page_index)
        ...
        print(f"总共收集到 {len(all_data)} 条商品库存数据")
        return all_data
    ...
    def upload_to_minio(self, file_path):
        ...
        try:
            ...
            # 构建MinIO路径
            minio_path = f"ods/erp/stock/dt={TODAY_STR}/{TODAY_STR}.parquet"
    ...
    def refresh_dremio_dataset(self):
        ...
        刷新Dremio数据集和反射
        """
        dataset_path = "minio.warehouse.ods.erp.stock"
...
def main():
    ...
    主函数
    """
    print("ERP商品库存数据采集程序启动...")
    print(f"目标日期: {TODAY_STR}")

    # 创建采集器
    collector = ERPStockCollector(profile="Default")
