...
def update_header_once(df: pd.DataFrame):
    """固定用 O 列（索引 14）作为状态列，每天第一次把表头改成 今天+下载状态"""
    new_status = f'{TODAY_STR}客服绩效下载状态'
    old_header = df.columns[14]          # O 列当前名字
...
def fetch_download_link(cookie: str):
    ...
    # 00:00:00 时间戳（秒）
    ts_start = int(datetime.combine(t3_day, time.min).timestamp())

    # 23:59:59 时间戳（秒）
    ts_end = int(datetime.combine(t3_day, time.max).timestamp())
...
def silent_download(url: str, shop_name: str, date_str: str = None):
    ...
    url: 下载链接
    shop_name: 店铺名称
    date_str: 日期字符串，如果为None则使用T-3日期
    """
    if date_str is None:
        date_str = TODAY_STR
    
    # 创建日期文件夹路径：D:\pdd\客服绩效文件存档\2025-09-25\
    date_dir = BASE_ARCHIVE_DIR / date_str
    date_dir.mkdir(parents=True, exist_ok=True)
    
    # 文件保存路径：D:\pdd\客服绩效文件存档\2025-09-25\店铺名称.xlsx
...
def upload_merged_file_to_minio(merged_file_path: str, date_str: str = None) -> bool:
    ...
    try:
        ...
        # 构建MinIO路径：ods/pdd/pdd_kpi_days/dt=日期/merged_kpi_data.parquet
        minio_path = f"ods/pdd/pdd_kpi_days/dt={date_str}/merged_kpi_data.parquet"
...
if __name__ == '__main__':
    ...
    if merge_success:
        # 读取合并后的文件
        merged_file_path = date_dir / f"客服绩效合并_{TODAY_STR}.xlsx"
